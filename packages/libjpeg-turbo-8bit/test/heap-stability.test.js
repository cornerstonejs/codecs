...
const isBuilt = existsSync(resolve(distDir, "libjpegturbowasm.js"))
...
describe("libjpeg-turbo-8bit wasm heap stability", { timeout: 120000 }, () => {
  let codec
  const encoded = readFileSync(resolve(fixturesDir, "jpeg/jpeg400jfif.jpg"))
  const raw = readFileSync(resolve(fixturesDir, "raw/jpeg400jfif.raw"))

  beforeAll(async () => {
    if (isBuilt) codec = await loadModule("../dist/libjpegturbowasm.js")
  })
  ...
  it.skipIf(!isBuilt)("repeated encode/delete cycles do not grow the heap", () => {
    const encodeOnce = () => {
      ...
      encoder.getDecodedBuffer({ width: 600, height: 800, bitsPerSample: 8, componentCount: 1, isSigned: false }).set(raw)
      encoder.encode()
      encoder.delete()
    }
