...
function encodeFile(codec, imageName, imageFrame, iterations = 1) {
  ...
  const result = codecHelper.encode(
    encoder,
...
function recodeFile(codec, imageName, imageFrame, iterations = 1) {
  ...
  for (let iteration = 0; iteration < iterations; iteration++) {
    ...
    console.timeEnd(`Encode ${imageName}`);
    // console.log(
    //   "WASM-recode encoded data",
    //   imageName,
    //   encodedBytes.length,
    //   imageFrame
    // );
    fs.writeFileSync(`./encoded-${imageName}.jxl`, encodedBytes);
    console.time(`Decode ${imageName}`);
    const decodeResult = codecHelper.decode(decoder, encodedBytes, 1);
    console.timeEnd(`Decode ${imageName}`);
    // console.log(
    //   "WASM-recode decoded data",
    //   imageName,
    //   decodeResult.frameInfo,
    //   decodeResult.pixels.length
    // );
    const { width, height, componentCount, bitsPerSample } = imageFrame;
    const sizeDelta =
      decodeResult.pixels.length -
      (width * height * componentCount * bitsPerSample) / 8;
    if (
      decodeResult.pixels.length !== uncompressedImageFrame.length &&
      Math.abs(sizeDelta) > 10
    ) {
      throw new Error(
        `Expected original (${uncompressedImageFrame.length}) and recoded lengths (${decodeResult.pixels.length} off by ${sizeDelta}) to be the same`
      );
    }
  }
  encoder.delete();
...
function main(codec) {
  ...
  recodeFile(
    codec,
    "VL1",
    { width: 756, height: 486, bitsPerSample: 8, componentCount: 3 },
    iterations
  );
  recodeFile(
    codec,
  ...
  recodeFile(
    codec,
    "VL4",
    { width: 2226, height: 1868, bitsPerSample: 8, componentCount: 3 },
    iterations
  );
  recodeFile(
    codec,
    "VL5",
    { width: 2670, height: 3340, bitsPerSample: 8, componentCount: 3 },
    iterations
  );
  recodeFile(
    codec,
